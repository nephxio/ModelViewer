...
{
	ModelViewerModel::ModelViewerModel(ModelViewerDevice& device, const std::vector<Vertex>& vertices) : modelViewerDevice { device }
	{
	...
	void ModelViewerModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(modelViewerDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<uint32_t>(bufferSize));
		vkUnmapMemory(modelViewerDevice.device(), vertexBufferMemory);
	}
